            ...
            switch(message.command) {
                ...
                case "USERNOTICE":
                    if (msgid === "resub") {
                        ...
                        var system_msg = _.get(message.tags["system-msg"].replace(/\\s/g, " "), '');

                        this.emits(["resub", "subanniversary"], [
                            [channel, username, months, msg, system_msg.includes("with Twitch Prime")],
                            [channel, username, months, msg, system_msg.includes("with Twitch Prime")]
                        ]);
                    }
                    break;
                ...
                case "USERSTATE":
                    ...
                    if (message.tags["user-type"] === "mod") {
                        ...
                        if (this.moderators[this.lastJoined].indexOf(this.username) < 0) { this.moderators[this.lastJoined].push(this.username); this.emit("mod", channel, this.username); }
                    }
                    ...
                    this.userstate[channel] = message.tags;
                    this.emit("userstate", channel, message.tags);
                    break;
                    ...
                    if (message.tags.username === "twitchnotify") {
                        ...
                        if (msg.includes("subscribed to")) {
                          // Ignore this feature.
                            ...
                            this.emit("subscription", channel, msg.split(" ")[0], msg.includes("with Twitch Prime"));
                        }
                    }
...
client.prototype._updateEmoteset = function _updateEmoteset(sets) {
    ...
    this.api({
        ...
        if (!err) { this.emotesets = body["emoticon_sets"] || {}; this.emit("emoteimages", this.emotesets); return this.emotesets; }

...
var api = function api(options, callback) {
    ...
    // Inside an extension -> we cannot use jsonp!
      ...
      // Overwrite default options with actual options!
      options.url = url;
      options = _.merge({ url: url, method: "GET", headers: {}, data: null }, options);
      // prepare request
      ...
      xhr.addEventListener("load", (ev) => {
        if(xhr.readyState == 4) {
          if(xhr.status != 200) {
            callback(xhr.status, null, xhr.response);
          } else {
            callback(null, null, xhr.response);
          }
        }
      });
      // submit request and send data
